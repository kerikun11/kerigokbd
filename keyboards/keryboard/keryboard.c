...
bool rgb_matrix_user_keyfunc(void) {
    for (uint8_t row = 0; row < MATRIX_ROWS; row++) {
        for (uint8_t col = 0; col < MATRIX_COLS; col++) {
            ...
            if (keycode == KC_NO || keycode == KC_TRNS) {
            ...
            } else if ((keycode >= S(JP_1) && keycode <= S(JP_0))
                    || (keycode >= KC_MINS && keycode <= KC_SLSH)
                    || (keycode >= S(KC_MINS) && keycode <= S(KC_SLSH))) {
                rgb_matrix_set_color(index, COLOR_SYMBOL);
            } else if (keycode == JP_BSLS || keycode == JP_UNDS
                    || keycode == JP_PIPE || keycode == JP_YEN) {
                rgb_matrix_set_color(index, COLOR_SYMBOL);
                ...
                rgb_matrix_set_color(index, COLOR_ARROW);
                    ...
                    || keycode == KC_LWIN  || keycode == KC_RWIN
                    || keycode == KC_ENT   || keycode == KC_SPC
                    || keycode == KC_TAB   || keycode == KC_BSPC
                    || keycode == KC_DEL   || keycode == KC_ESC) {
                rgb_matrix_set_color(index, COLOR_SPECIAL);
            } else if (keycode == RCTL_T(JP_MHEN) || keycode == RSFT_T(JP_HENK)
                    || keycode == RWIN_T(JP_HENK) || keycode == LALT_T(JP_MHEN)) {
                rgb_matrix_set_color(index, COLOR_SPECIAL);
