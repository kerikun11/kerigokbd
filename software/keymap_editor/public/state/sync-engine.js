...
export class SyncEngine {
  ...
  /** Writes one key's keycode immediately (optimistic update, then confirmed by the device's ack). */
  async writeKeycode(layer, keyIndex, value) {
    const { layout } = this.#store;
    const [row, col] = layout.keys[keyIndex].matrix;
    const previous = this.#store.keycodeAt(layer, keyIndex);
    this.#store.setKeycodeAt(layer, keyIndex, value);
    this.#store.setPending(layer, row, col, true);
    try {
      await this.#client.setKeycode(layer, row, col, value);
    } catch (error) {
      this.#store.setKeycodeAt(layer, keyIndex, previous);
      throw error;
    } finally {
