...
- **早見表 / 編集画面の切り替え**
  - 設定ペイン右上の「表示 | 編集」で、全レイヤー重ね合わせの早見表(デフォルト表示)と、実機の値を書き換える編集画面を切り替える。接続前はファームウェアの初期状態(`defaults-*.js`)、接続後は実機の現在の値を表示
...
- **実機との同期(WebHID + VIA)**
  ...
  - 編集画面では割り当ての種類を色分けし、凡例に表示する(LT: 青、MO: 緑、TO: 紫、MT: 黄土、修飾キー同時押し: ピンク)
