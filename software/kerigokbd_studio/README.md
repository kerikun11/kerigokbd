...
- **実機との同期(WebHID + VIA)**
  ...
  - 長押し: ピッカーの「長押し」行で「なし / MO (レイヤーのみ) / LT (タップ+レイヤー) / MT (タップ+修飾キー)」を選ぶ。MO・LTはレイヤーのチップ、MTは修飾キーのチェックボックス(「右側 (R)」で右側)で指定し、選んだ時点で現在のキーに反映される。LT・MTは現在のタップキーをそのまま包み、以降に選んだ基本キーがタップ動作になる。MOはタップ動作を持たないため、キーを選ぶと通常のキーに戻る。TO・DF・TGなどは「レイヤー動作」タブから割り当てる。今のレイヤーを固定するLayer Lock(`QK_LLCK`)は「特殊」タブから割り当てる(ファームウェアの`LAYER_LOCK_ENABLE = yes`が必要)
  ...
  - キーの交換: キーを選んでピッカー上部の「交換」を押し、交換先のキーをクリックすると、2つのキーの割り当てを入れ替えて未書き込みの変更にする。レイヤータブで切り替えれば別のレイヤーのキーとも交換できる。交換元をもう一度クリック、Esc、または「交換を中止」で取りやめる
