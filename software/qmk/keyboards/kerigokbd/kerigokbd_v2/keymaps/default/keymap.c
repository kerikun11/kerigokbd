// Copyright 2025 KERI's Lab
// SPDX-License-Identifier: GPL-2.0-or-later
// keymap.c for KERIgoKBD v2

#include QMK_KEYBOARD_H
#include "kerigokbd.h"

// clang-format off
#ifdef LAYOUT_split_6_7_7_4
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  [KGL_MAIN] = LAYOUT_split_6_7_7_4( /* Default Layer */
    KC_TAB , KC_Q   , KC_W   , KC_E   , KC_R   , KC_T   ,          /**/          KC_Y   , KC_U   , KC_I   , KC_O   , KC_P   , KC_BSPC,
    KC_LCTL, KC_A   , KC_S   , KC_D   , KC_F   , KC_G   , KG_CTTL, /**/ KG_CTTR, KC_H   , KC_J   , KC_K   , KC_L   , JP_MINS, KC_ENT ,
    KC_LSFT, KC_Z   , KC_X   , KC_C   , KC_V   , KC_B   , KG_CTBL, /**/ KG_CTBR, KC_N   , KC_M   , JP_COMM, JP_DOT , JP_SLSH, KC_DEL ,
                               KG_LWIN, KG_LALT, KG_NUM , KG_ESC , /**/ KG_SPC , KG_FUN , XXXXXXX, XXXXXXX
  ),
  [KGL_NUM] = LAYOUT_split_6_7_7_4( /* Numbers and Symbols Layer */
    _______, JP_EXLM, JP_DQUO, JP_HASH, JP_DLR , JP_PERC,          /**/          JP_AMPR, JP_QUOT, JP_EQL , JP_TILD, JP_PIPE, _______,
    _______, KC_1   , KC_2   , KC_3   , KC_4   , KC_5   , KC_PSCR, /**/ KC_VOLU, KC_6   , KC_7   , KC_8   , KC_9   , KC_0   , _______,
    KC_LWIN, JP_GRV , JP_AT  , JP_SCLN, JP_COLN, JP_UNDS, KG_APRS, /**/ KC_VOLD, JP_CIRC, JP_PLUS, JP_ASTR, JP_DOT , JP_BSLS, _______,
                               _______, _______, _______, _______, /**/ _______, _______, XXXXXXX, XXXXXXX
  ),
  [KGL_FUN] = LAYOUT_split_6_7_7_4( /* Functions and Navigation Layer */
    _______, KC_F1  , KC_F2  , KC_F3  , KC_F4  , KC_F5  ,          /**/          KG_MWLL, KG_MWLD, KG_MWLU, KG_MWLR, MS_BTN2, _______,
    _______, JP_LPRN, JP_LCBR, JP_RCBR, JP_LBRC, JP_RBRC, KC_F11 , /**/ MS_BTN3, KC_LEFT, KC_DOWN, KC_UP  , KC_RGHT, JP_RPRN, _______,
    KC_LSFT, KC_F6  , KC_F7  , KC_F8  , KC_F9  , KC_F10 , KC_F12 , /**/ MS_BTN2, KG_MSL , KG_MSD , KG_MSU , KG_MSR , MS_BTN1, _______,
                               _______, _______, _______, _______, /**/ _______, _______, XXXXXXX, XXXXXXX
  ),
  [KGL_EXT] = LAYOUT_split_6_7_7_4( /* Extra Layer */
    JP_ZKHK, KC_F1  , KC_F2  , KC_F3  , KC_F4  , KC_F5  ,          /**/          KC_P7  , KC_P8  , KC_P9  , KC_PMNS, KC_NUM , _______,
    _______, A(KC_1), A(KC_2), A(KC_3), A(KC_4), A(KC_5), KC_F11 , /**/ KC_PSLS, KC_P4  , KC_P5  , KC_P6  , KC_PPLS, KC_UP  , KC_PENT,
    _______, C(KC_Z), C(KC_X), C(KC_C), KG_MWLL, KG_MWLR, KC_F12 , /**/ KC_PAST, KC_P1  , KC_P2  , KC_P3  , KC_LEFT, KC_DOWN, KC_RGHT,
                               KC_SLEP, XXXXXXX, XXXXXXX, KG_TO_M, /**/ KC_PDOT, KC_P0  , XXXXXXX, XXXXXXX
  ),
  [KGL_RES] = LAYOUT_split_6_7_7_4( /* Reserved Layer */
    XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX,          /**/          XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX,
    XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, /**/ XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX,
    XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, /**/ XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX,
                               XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, /**/ XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX
  ),
  [KGL_CONF] = LAYOUT_split_6_7_7_4( /* Config Layer */
    XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX,          /**/          XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX,
    XXXXXXX, RM_SATU, RM_HUEU, RM_VALU, RM_SPDU, RM_NEXT, QK_LLCK, /**/ QK_LLCK, RM_NEXT, RM_SPDU, RM_VALU, RM_HUEU, RM_SATU, XXXXXXX,
    QK_BOOT, RM_SATD, RM_HUED, RM_VALD, RM_PREV, RM_SPDD, RM_TOGG, /**/ RM_TOGG, RM_PREV, RM_SPDD, RM_VALD, RM_HUED, RM_SATD, QK_BOOT,
                               XXXXXXX, XXXXXXX, KG_TO_N, KG_TO_E, /**/ KG_TO_E, KG_TO_F, XXXXXXX, XXXXXXX
  ),
  [KGL_AM] = LAYOUT_split_6_7_7_4( /* Auto Mouse Layer (PointingDevice only) */
    KC_TAB , XXXXXXX, XXXXXXX, MS_BTN3, XXXXXXX, XXXXXXX,          /**/          XXXXXXX, XXXXXXX, MS_BTN3, XXXXXXX, XXXXXXX, KC_BSPC,
    KC_LCTL, XXXXXXX, MS_BTN1, KG_SCRL, MS_BTN2, XXXXXXX, XXXXXXX, /**/ XXXXXXX, XXXXXXX, MS_BTN1, KG_SCRL, MS_BTN2, XXXXXXX, KC_ENT ,
    KC_LSFT, XXXXXXX, XXXXXXX, KG_ZOOM, XXXXXXX, XXXXXXX, XXXXXXX, /**/ XXXXXXX, XXXXXXX, XXXXXXX, KG_ZOOM, XXXXXXX, XXXXXXX, KC_DEL ,
                               KC_LWIN, KC_LALT, MS_BTN1, KC_ESC , /**/ XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX
  ),
};
#endif
// clang-format on

#if 0
void keyboard_post_init_user(void) {
    debug_enable   = true;
    debug_matrix   = true;
    debug_keyboard = true;
    debug_mouse    = true;
}
#endif
